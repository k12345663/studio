...
const QuestionAnswerPairSchema = z.object({
  ...
  answer: z.string().describe("A model answer as 3-4 concise bullet points. Each bullet point MUST serve as a **general example of a strong answer** from an **interviewer's perspective** – basic, clear, and easy to judge. These points should guide the interviewer on **what key aspects a strong candidate's answer should cover**, and MUST be informed by proficiency relevant to the candidate's specific work experience, past experiences (from resume/context, including projects and their specific details like tech stack, goals, accomplishments, challenges, educational background, academic achievements) and background by EXPLICITLY referencing key terms, skills, projects, or experiences from the Job Description AND/OR the Candidate Resume/Context (including specific project details, educational background, and past work experiences). Highlight positive indicators a recruiter should look for. For 'Tell me about yourself', if a resume is provided, the model answer MUST be specifically tailored to the candidate's resume. It should outline 3-4 key bullet points a strong candidate, based on THEIR specific resume (considering their work history, projects, educational background, and academic achievements), would ideally cover. **Crucially, this model answer should be written from the interviewer's perspective, guiding them on what points covered from the candidate's resume would constitute a strong, relevant introduction, rather than being a direct script for the candidate to recite.** For resume project deep-dive questions, the model answer should guide the interviewer on what to listen for regarding project goals, tech stack, accomplishments, and challenges."),
...
const generateInterviewKitPrompt = ai.definePrompt({
  ...
  prompt: `You are a seasoned recruiter with 25 years of experience. Your primary task is to act as an expert interview designer and tailor a comprehensive series of questions and a full interview kit for the candidate, based on their specific Job Description, their profile (derived from the Candidate Resume including their projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), and any provided Candidate Experience Context.
    ...
    const validatedOutput: GenerateInterviewKitOutput = {
      competencies: (output.competencies || []).map(comp => ({
        ...
        questions: (comp.questions || []).map(q => ({
          ...
          answer: q.answer || "Missing model answer. AI should provide general example answer (3-4 bullet points, highlighting key points to cover from interviewer's perspective) informed by JD/resume/context. For 'Tell me about yourself', it should guide on what a candidate should cover from their resume.",
