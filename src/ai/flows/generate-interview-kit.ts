...
3.  **For EACH question, provide all fields as specified in the output schema**:
    ...
    *   \`answer\`: This is the most critical part of your role as a recruiter companion. The model answer is **FOR THE INTERVIEWER'S USE** and must have the "aura" of what an experienced recruiter expects.
        *   **Format**: 3-4 concise bullet points.
        ...
        *   **Content for some general questions**: For some general technical or behavioral questions, the model answer MUST focus on **FUNDAMENTAL principles and CORE CONCEPTS broken down SIMPLY**. For example, for an OOP question, guide the interviewer to listen for the 4 pillars. For a systems design question, guide them to check for concepts like scalability, reliability, and trade-offs. The goal is to help the recruiter evaluate the candidate's structured thinking and grasp of first principles, which is a stronger signal of expertise than just reciting facts. For some general technical or behavioral questions, key points in model answers may focus on fundamental concepts or best practices rather than strict JD mapping for every point.
