...
Return JSON object per output schema. Populate all fields. Goal: logically sequenced kit, relevant tailored questions (from Unstop profile/resume text [treated as full text from PDF/DOC]: projects, tech stack, goals, accomplishments, challenges, education, past experiences), concise judgeable model answers (recruiter's perspective, brief and crisp key points, each with indicative score contribution using whole numbers or small clear ranges, collective contributions conceptually aligning to a full 10-point answer; "Tell me about yourself" model answer tailored from Unstop/resume to help a non-technical recruiter assess candidate's intro against their profile), contextual well-defined rubric for non-technical recruiters that accounts for emergent candidate information.
`,
});
