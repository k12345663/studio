...
const QuestionSchema = z.object({
  ...
  text: z.string().describe('The text of the question. Ensure it is insightful and specific, considering JD and candidate profile (resume including projects, their tech stack, goals, accomplishments, challenges, educational background, academic achievements, past work experiences & context).'),
  modelAnswer: z.string().describe("An example answer for the question, presented as 3-4 concise bullet points. Each bullet MUST serve as a **general example of a strong answer** from an **interviewer's perspective**, be basic, clear, and easy to judge. It should demonstrate proficiency for the target experience level (drawing from candidate's specific work experience, past experiences from resume/context, including projects and their specifics like tech stack, goals, accomplishments, challenges, educational background, academic achievements), guide the interviewer on **what key aspects a strong candidate's answer should cover**, and EXPLICITLY reference terms, skills, projects, or experiences from the Job Description, Candidate Resume, or context if appropriate. Ensure this format is maintained if modified. For 'Tell me about yourself', if a resume is provided, the model answer MUST be specifically tailored to outline 3-4 key points a strong candidate would ideally cover based on THEIR specific resume (including work history, projects, educational background, and academic achievements). **Crucially, this 'Tell me about yourself' model answer should be written from the interviewer's perspective, guiding them on what points covered from the candidate's resume would constitute a strong, relevant introduction, rather than being a direct script for the candidate to recite.** For resume project deep-dive questions, the model answer should guide the interviewer on what to listen for regarding project goals, tech stack, accomplishments, and challenges. The candidate's resume (including projects, their tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences) should be a key reference for validating and refining model answers."),
...
const CustomizeInterviewKitInputSchema = z.object({
  ...
  candidateResume: z.string().optional().describe('The full text of the candidate\'s resume that was used and should be considered for refinements. This is a key reference document for tailoring, including specific projects mentioned (their tech stack, goals, accomplishments, challenges), educational background, academic achievements, and past work experiences. Analyze it deeply.'),
  competencies: z.array(CompetencySchema).describe('Array of core competencies, potentially with importance, questions with category, difficulty/time. User edits are reflected here. May include "Tell me about yourself". Competencies should be informed by the holistic analysis of JD and candidate profile (including educational background and academic achievements from resume).'),
...
const customizeInterviewKitPrompt = ai.definePrompt({
  ...
  prompt: `You are a seasoned recruiter with 25 years of experience. You will be given an interview kit previously generated for a candidate. This kit was based on a Job Description, and potentially a Candidate Resume (detailing projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences) and/or Candidate Experience Context. The kit includes questions, model answers, competency importance, question categories, difficulty levels, and estimated times. The hiring manager (the user) has made edits to this kit. Your task is to review these edits and refine the entire kit.
Critical: Before refining any content, take the time to thoroughly analyze and synthesize ALL provided details: the Job Description (primary source), the Candidate Resume (primary source if provided, analyze it deeply including specific projects, their tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), any Candidate Experience Context, AND the user's edits. In your role as this highly experienced recruiter, ensure the refined kit remains highly contextual and tailored to the candidate based on all original inputs (Job Description, their profile from the Candidate Resume including projects and tech stack, and any Candidate Experience Context) AND the user's explicit edits. Your entire output must be deeply informed by this holistic understanding.

Job Description (Primary Source, for context):
{{{jobDescription}}}

{{#if candidateResume}}
Candidate Resume (Primary Source, for context and primary reference, including projects, their tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences):
{{{candidateResume}}}
{{/if}}
...
2.  If the recruiter modified a question's text or model answer, ensure the updated content remains high quality, insightful, and relevant to the JD and candidate profile (resume/projects/education/context).
    *   Model answers MUST be 3-4 concise bullet points, serving as **general examples of a strong answer** from an **interviewer's perspective**, be basic, clear, and easy to judge. They should guide the interviewer on **what key aspects a strong candidate's answer should cover**, and EXPLICITLY reference specific terms, skills, projects, or experiences from the Job Description AND/OR Candidate Resume/Context (reflecting candidate's work and past experiences, including specific project details like tech stack, goals, accomplishments, challenges, educational background, and academic achievements).
...
6.  Ensure all output fields (importance, category, difficulty, estimatedTimeMinutes, 3-4 bullet model answers highlighting key points to cover from an interviewer's perspective and referencing JD/resume/projects/education/context and their specifics like tech stack, goals, accomplishments, challenges, serving as general examples of strong answers) are present for all competencies and questions.
...
const customizeInterviewKitFlow = ai.defineFlow(
  ...
  async input => {
    ...
    const validatedOutput = {
      ...
      competencies: output.competencies.map(comp => ({
        ...
        questions: comp.questions.map(q => ({
          ...
          modelAnswer: q.modelAnswer || "Missing model answer. AI should provide general example answer (3-4 bullet points, highlighting key points to cover from interviewer's perspective) informed by JD/resume/context. For 'Tell me about yourself', it should guide on what a candidate should cover from their resume.",
        })),
      })),
    ...
    if (Math.abs(finalSum - 1.0) > 0.001 && validatedOutput.rubricCriteria.length > 0) {
        ...
        if (lastCrit.weight < 0) { // This should ideally not happen with prior checks, but as a safeguard
            lastCrit.weight = 0;
