...
const CustomizeInterviewKitInputSchema = z.object({
  ...
  candidateResumeDataUri: z.string().optional().describe("The data URI of the candidate's resume file (PDF or DOCX) that was used. If provided, AI should consider its content for refinements."),
...
const CustomizeInterviewKitOutputSchema = z.object({
  competencies: z.array(CompetencySchema).describe("Array of customized core competencies, including importance, and questions with category, difficulty/time. Quality of questions and answers should be maintained or enhanced, with answers from an interviewer's perspective (3-4 bullet points, which should be very brief and crisp key words/phrases for the recruiter's quick reference, highlighting key points to cover, each with indicative contribution to score using whole numbers or small ranges of whole numbers, basic, clear, and easy for a non-technical recruiter to judge, and where the collective contributions conceptually align with a full 10-point answer) referencing JD and candidate profile (Unstop Profile/Resume File Content [AI to analyze if provided] including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, past work experiences & context, and how to evaluate relevant details not on resume). For 'Tell me about yourself', model answers should remain interviewer-focused guides based on the Unstop Profile/Resume file content (including candidate's work history, projects, education, academic achievements), ensuring a non-technical recruiter can assess relevance. Questions should aim for a logical flow where appropriate."),
...
*   **Generate Insightful Questions (or refine existing ones to reflect this):** Formulate questions that directly address this. Encourage the candidate to bridge this perceived gap by elaborating on their project work. For example:
...
6.  Ensure all output fields (importance, category, difficulty, estimatedTimeMinutes, model answers from interviewer's perspective covering brief and crisp key points with indicative score contributions using whole numbers or small clear ranges of whole numbers and referencing JD/Unstop Profile/Resume File Content [AI to analyze if provided] and considering emergent details, collective contributions conceptually aligning to a full 10-point answer, **simple and clear for non-technical evaluation**) are present for all competencies and questions.
