...
const CustomizeInterviewKitOutputSchema = z.object({
  competencies: z.array(CompetencySchema).describe("Array of customized core competencies, including importance, and questions with category, difficulty/time. Quality of questions and answers should be maintained or enhanced, with answers from an interviewer's perspective (3-4 bullet points highlighting key points to cover, basic, clear, and easy for a non-technical recruiter to judge) referencing JD and candidate profile (Unstop Profile/Resume Text including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, past work experiences & context, and how to evaluate relevant details not on resume). 'Tell me about yourself' model answers should remain interviewer-focused guides based on the Unstop Profile/Resume text, ensuring a non-technical recruiter can assess relevance. Questions should aim for a logical flow where appropriate."),
...
2.  If the recruiter modified a question's text or model answer, ensure the updated content remains high quality, insightful, and relevant to the JD and candidate profile (Unstop Profile/Resume Text/projects/education/context).
    *   Model answers MUST be FROM THE INTERVIEWER'S PERSPECTIVE, 3-4 concise bullet points, outlining KEY POINTS A CANDIDATE SHOULD COVER for a strong answer, making it **exceptionally easy for a non-technical recruiter to judge**. They should serve as **general examples of strong answers**, reflecting core concepts (e.g., if OOP is asked, the answer should guide the recruiter to check for the 4 pillars: Abstraction, Encapsulation, Inheritance, Polymorphism, clearly listing them), yet EXPLICITLY reference specific terms, skills, projects, or experiences from the Job Description AND/OR Candidate's Unstop Profile/Resume Text (reflecting candidate's work and past experiences, including specific project details like tech stack, goals, accomplishments, challenges, educational background, academic achievements). Include guidance on evaluating real-life examples and relevant information shared by the candidate not present on the resume using a note like: 'Note: If the candidate provides relevant real-life examples or discusses experiences/skills not detailed on their resume/profile but clearly relevant to the role, this can indicate greater depth, initiative, or broader experience. The interviewer should assess the relevance and substance of such unstated information against the job requirements.'
...
6.  Ensure all output fields (importance, category, difficulty, estimatedTimeMinutes, model answers from interviewer's perspective covering key points and referencing JD/Unstop Profile/Resume Text and considering emergent details, **simple and clear for non-technical evaluation**) are present for all competencies and questions.
    ...
    const validatedOutput = {
      ...
      competencies: output.competencies.map(comp => ({
        ...
        questions: comp.questions.map(q => ({
          ...
          modelAnswer: q.modelAnswer || "Missing model answer. AI should provide guidance from an interviewer's perspective on key points the candidate should cover, informed by JD/Unstop Profile/Resume Text/context (and how to evaluate relevant emergent details), making it easy for a non-technical recruiter to judge. For 'Tell me about yourself', it should guide on what a candidate should cover from their Unstop Profile/Resume Text to help a non-technical recruiter assess relevance.",
        })),
      })),
