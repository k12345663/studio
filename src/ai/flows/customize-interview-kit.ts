...
const QuestionSchema = z.object({
  ...
  text: z.string().describe('The text of the question. Ensure it is insightful and specific, considering JD, Unstop Profile (compulsory input, if link provided and conceptually analyzable) and Candidate Resume Text (optional input, including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, past work experiences & context).'),
  modelAnswer: z.string().describe("A model answer from the INTERVIEWER'S PERSPECTIVE, presented as 3-4 concise bullet points. Each bullet MUST outline KEY POINTS A CANDIDATE SHOULD COVER for a strong answer, making it easy for a non-technical recruiter to judge. It should serve as a general example of a strong answer, reflecting core concepts, yet be informed by the candidate's profile (Unstop/resume text) and JD by EXPLICITLY referencing terms, skills, projects, or experiences from the Job Description, Candidate's Unstop Profile/Resume Text, or context. Example: 'Interviewer should note if candidate covers: 1. Core concept A. 2. Application B related to [JD skill/Unstop Profile project/Resume project].' Include a brief note: 'Note: If the candidate provides relevant real-life examples or discusses experiences/skills not detailed on their resume/profile but clearly relevant to the role, this can indicate greater depth, initiative, or broader experience. The interviewer should assess the relevance and substance of such unstated information against the job requirements.' For 'Tell me about yourself', if Unstop profile/resume text is available, the model answer MUST guide the recruiter on key points from the candidate's specific background (work history, projects from Unstop/resume, education) that would constitute a strong introduction, written from the interviewer's perspective. The candidate's Unstop profile/resume text (including projects, past work experiences, educational background) should be a key reference for validating and refining model answers."),
...
const CustomizeInterviewKitInputSchema = z.object({
  ...
  unstopProfileLink: z.string().optional().describe("The candidate's Unstop profile link (compulsory input). This is a primary source material if provided; AI should (conceptually) analyze the content of this profile deeply if it was used and should be considered for refinements."),
  candidateResumeText: z.string().optional().describe("The full text extracted from the candidate's resume (e.g., from an uploaded PDF/DOC or pasted text - optional input) that was used and should be considered for refinements. This is a key reference document for tailoring, including specific projects mentioned (their tech stack, goals, accomplishments, challenges), educational background, academic achievements, and past work experiences. Analyze it deeply if provided."),
...
const customizeInterviewKitPrompt = ai.definePrompt({
  ...
  prompt: `You are a highly experienced hiring manager and recruiter with 25 years of experience, acting as a supportive companion to the interviewer. You specialize in refining interview kits for various roles, ensuring they are usable by recruiters who may not be technical experts. You will be given an interview kit previously generated. This kit was based on a Job Description, an Unstop Profile Link (compulsory), and potentially Candidate Resume Text (optional, detailing projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), and/or Candidate Experience Context. The kit includes questions, model answers, competency importance, question categories, difficulty levels, and estimated times. The hiring manager (the user) has made edits to this kit. Your task is to review these edits and refine the entire kit.
CRITICAL: Before refining any content, take the time to thoroughly analyze and synthesize ALL provided details: the Job Description (primary source), the Unstop Profile Link (primary source - COMPULSORY, conceptually analyze its content), the Candidate Resume Text (primary source - OPTIONAL, if provided, analyze it deeply including specific projects, their tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), any Candidate Experience Context, AND the user's edits. Ensure the refined kit remains highly contextual and tailored based on all original inputs AND the user's explicit edits. Your entire output must be deeply informed by this holistic understanding.

Job Description (Primary Source, for context):
{{{jobDescription}}}

Unstop Profile Link (Primary Source - COMPULSORY, for context and primary reference; conceptually analyze content):
{{{unstopProfileLink}}}

{{#if candidateResumeText}}
Candidate Resume Text (Primary Source - OPTIONAL, for context and primary reference, including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences):
{{{candidateResumeText}}}
{{/if}}
...
Based on the recruiter's modifications and a holistic understanding of the original Job Description, Unstop Profile Link (as a key reference, conceptually analyze its content), Candidate Resume Text (as a key reference, including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), and Candidate Experience Context:
...
2.  If the recruiter modified a question's text or model answer, ensure the updated content remains high quality, insightful, and relevant to the JD and candidate profile (Unstop Profile/Resume Text/projects/education/context).
    *   Model answers MUST be FROM THE INTERVIEWER'S PERSPECTIVE, 3-4 concise bullet points, outlining KEY POINTS A CANDIDATE SHOULD COVER for a strong answer, making it easy for a non-technical recruiter to judge. They should serve as general examples of strong answers, reflecting core concepts, yet EXPLICITLY reference specific terms, skills, projects, or experiences from the Job Description AND/OR Candidate's Unstop Profile/Resume Text (reflecting candidate's work and past experiences, including specific project details like tech stack, goals, accomplishments, challenges, educational background, academic achievements). Include the note: 'Note: If the candidate provides relevant real-life examples or discusses experiences/skills not detailed on their resume/profile but clearly relevant to the role, this can indicate greater depth, initiative, or broader experience. The interviewer should assess the relevance and substance of such unstated information against the job requirements.'
    *   The candidate's Unstop Profile/Resume Text (including projects, past work experiences, educational background, academic achievements) should be a key reference for validating and refining model answers.
    *   For "Tell me about yourself," the model answer should continue to guide the interviewer on what points a strong candidate, based on their Unstop Profile/Resume text (including education and academic achievements), would cover, written from the interviewer's perspective.
...
6.  Ensure all output fields (importance, category, difficulty, estimatedTimeMinutes, model answers from interviewer's perspective covering key points and referencing JD/Unstop Profile/Resume Text and considering emergent details) are present for all competencies and questions.

Return the fully customized and refined interview kit in the specified JSON format. The goal is a polished, consistent, and high-quality interview kit that intelligently incorporates the recruiter's edits and adheres to all formatting and contextual requirements based on the JD, Unstop Profile Link, Candidate Resume Text (including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), and any other candidate context, making it highly usable for non-technical recruiters and adaptable to information shared during the interview.
`,
});
    ...
    const validatedOutput = {
      ...
      competencies: output.competencies.map(comp => ({
        ...
        questions: comp.questions.map(q => ({
          ...
          modelAnswer: q.modelAnswer || "Missing model answer. AI should provide guidance from an interviewer's perspective on key points the candidate should cover, informed by JD/Unstop Profile/Resume Text/context (and how to evaluate relevant emergent details). For 'Tell me about yourself', it should guide on what a candidate should cover from their Unstop Profile/Resume Text.",
        })),
      })),
      rubricCriteria: output.rubricCriteria.map(rc => ({
          ...rc,
          name: rc.name || "Unnamed Criterion (must be well-defined, distinct, high-quality, actionable, measurable, and contextually reference JD/Unstop Profile/Resume Text, considering emergent details, for comprehensive evaluation by a non-technical recruiter). AI should refine this.",
