...
const CompetencySchema = z.object({
  ...
  questions: z.array(QuestionSchema).describe('Array of questions for the competency. Ensure questions and answers maintain high quality if modified or regenerated, referencing JD and candidate profile (Unstop Profile Link, and especially the Resume Text, including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences). Try to maintain a logical sequence of questions within competencies if edits allow.'),
});
...
const CustomizeInterviewKitInputSchema = z.object({
  ...
  unstopProfileLink: z.string().optional().describe("The candidate's Unstop profile link. This is a primary source material if provided; AI should (conceptually) analyze the content of this profile deeply if it was used and should be considered for refinements."),
  candidateResumeText: z.string().optional().describe("The full text extracted from the candidate's resume (e.g., from an uploaded PDF/DOC or pasted text) that was used and should be considered for refinements. This is a key reference document for tailoring, including specific projects mentioned (their tech stack, goals, accomplishments, challenges), educational background, academic achievements, and past work experiences. Analyze it deeply."),
...
const customizeInterviewKitPrompt = ai.definePrompt({
  ...
  prompt: `You are a highly experienced hiring manager and recruiter with 25 years of experience, specializing in refining interview kits for various roles, ensuring they are usable by recruiters who may not be technical experts. You will be given an interview kit previously generated. This kit was based on a Job Description, and potentially an Unstop Profile Link, Candidate Resume Text (detailing projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), and/or Candidate Experience Context. The kit includes questions, model answers, competency importance, question categories, difficulty levels, and estimated times. The hiring manager (the user) has made edits to this kit. Your task is to review these edits and refine the entire kit.
CRITICAL: Before refining any content, take the time to thoroughly analyze and synthesize ALL provided details: the Job Description (primary source), the Unstop Profile Link (primary source if provided, conceptually analyze its content), the Candidate Resume Text (primary source if provided, analyze it deeply including specific projects, their tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences), any Candidate Experience Context, AND the user's edits. Ensure the refined kit remains highly contextual and tailored based on all original inputs AND the user's explicit edits. Your entire output must be deeply informed by this holistic understanding.

Job Description (Primary Source, for context):
{{{jobDescription}}}

{{#if unstopProfileLink}}
Unstop Profile Link (Primary Source, for context and primary reference; conceptually analyze content):
{{{unstopProfileLink}}}
{{/if}}

{{#if candidateResumeText}}
Candidate Resume Text (Primary Source, for context and primary reference, including projects, tech stack, goals, accomplishments, challenges, educational background, academic achievements, and past work experiences):
{{{candidateResumeText}}}
{{/if}}
...
2.  If the recruiter modified a question's text or model answer, ensure the updated content remains high quality, insightful, and relevant to the JD and candidate profile (Unstop Profile/Resume Text/projects/education/context).
    *   Model answers MUST be FROM THE INTERVIEWER'S PERSPECTIVE, 3-4 concise bullet points, outlining KEY POINTS A CANDIDATE SHOULD COVER for a strong answer, making it easy for a non-technical recruiter to judge. They should serve as general examples of strong answers, reflecting core concepts, yet EXPLICITLY reference specific terms, skills, projects, or experiences from the Job Description AND/OR Candidate's Unstop Profile/Resume Text (reflecting candidate's work and past experiences, including specific project details like tech stack, goals, accomplishments, challenges, educational background, academic achievements). Include notes for the interviewer regarding real-life examples if appropriate.
...
6.  Ensure all output fields (importance, category, difficulty, estimatedTimeMinutes, model answers from interviewer's perspective covering key points and referencing JD/Unstop Profile/Resume Text) are present for all competencies and questions.
    ...
    const validatedOutput = {
      ...
      competencies: output.competencies.map(comp => ({
        ...
        questions: comp.questions.map(q => ({
          ...
          modelAnswer: q.modelAnswer || "Missing model answer. AI should provide guidance from an interviewer's perspective on key points the candidate should cover, informed by JD/Unstop Profile/Resume Text/context. For 'Tell me about yourself', it should guide on what a candidate should cover from their Unstop Profile/Resume Text.",
        })),
      })),
      rubricCriteria: output.rubricCriteria.map(rc => ({
          ...rc,
          name: rc.name || "Unnamed Criterion (must be well-defined, distinct, high-quality, actionable, measurable, and contextually reference JD/Unstop Profile/Resume Text for comprehensive evaluation by a non-technical recruiter). AI should refine this.",
