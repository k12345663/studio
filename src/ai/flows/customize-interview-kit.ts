...
const QuestionSchema = z.object({
  ...
  modelAnswer: z.string().describe("A model answer FOR THE INTERVIEWER'S USE, presented as 4 concise bullet points. **For most questions, these bullet points for the recruiter MUST BE EXTREMELY BRIEF AND CRISP, ideally just ABSOLUTELY ESSENTIAL KEYWORDS or CRITICAL SHORT PHRASES, serving as a rapid mental checklist.** Each bullet point MUST outline KEY POINTS A CANDIDATE SHOULD COVER for a strong answer, making it **exceptionally easy for a non-technical recruiter to quickly assess**. Furthermore, each bullet point MUST also include a textual suggestion of its indicative weight or contribution (e.g., 'approx. 2-3 points', 'around 4 points') towards the question's total 10-point score, using whole numbers or small, clear ranges of whole numbers. **For some general technical or behavioral questions, especially those testing a core JD requirement like OOP, they must reflect CORE CONCEPTS broken down into their most FUNDAMENTAL, EASILY DIGESTIBLE PARTS (e.g., if OOP is asked, the answer should guide the recruiter to check for: '1. Abstraction mentioned. 2. Encapsulation explained. 3. Inheritance example. 4. Polymorphism concept.', clearly listing these pillars).** EXPLICITLY reference key terms from JD AND/OR Candidate's Unstop Profile/Resume File Content when crucial. Include guidance on evaluating real-life examples shared by the candidate not present on the resume. For the \"Tell me about yourself\" question, the model answer should be a more descriptive guide for the interviewer (not just keywords), based on the candidate's specific profile, framed to help a non-technical recruiter assess relevance."),
...
const CustomizeInterviewKitOutputSchema = z.object({
  competencies: z.array(CompetencySchema).describe("Array of customized core competencies, including importance, and questions with category, difficulty/time. Quality of questions and answers should be maintained or enhanced. For most questions, answers for the interviewer must be EXTREMELY BRIEF and CRISP keywords/short phrases as key points, each with indicative score contribution, referencing JD/candidate profile. For some general questions, especially those on core JD skills, they must focus on FUNDAMENTAL concepts broken down SIMPLY for non-technical evaluation. For 'Tell me about yourself', model answers should be a more descriptive interviewer-focused guide based on the Unstop Profile/Resume file content (including candidate's work history, projects, education, academic achievements), ensuring a non-technical recruiter can assess relevance. All answers must guide on evaluating relevant details not on the resume."),
...
*   **Bridging Background & Domain Differences:**
    ...
    *   If candidate is *transitioning career domains* (e.g., QA to DevOps) (e.g., TC-EXPPR-009): Generate questions probing practical application of newly acquired skills and how their prior domain experience provides unique strengths.
...
*   **Handling Vague/Sparse Inputs (Job Description and Candidate Profile):**
    ...
    *   If both original JD and candidate profile were sparse, or resume content was unparseable (e.g., TC-EMPTY\\*, TC-RESUN\\*, \\\`TC-RESUME-IMGOCR-097\\\`, \\\`TC-RESUME-FMT-086\\\`): Generate more fundamental questions. The *interviewer notes* within model answers should reflect that questions are broader due to ongoing input limitations or data extraction challenges from resume.
...
Based on the recruiter's modifications and a holistic understanding of the original Job Description, Unstop Profile Link (**conceptually treated as live profile**), the content of the Candidate Resume File (if provided), and Candidate Experience Context:
1.  Preserve all existing IDs for competencies and questions.
2.  If the recruiter modified a question's text or model answer, ensure the updated content remains high quality, insightful, and relevant. Apply the "astute evaluator" principles described above.
    *   **Model answers MUST be FOR THE INTERVIEWER'S USE.** For most questions, they must be 4 concise bullet points of **EXTREMELY BRIEF AND CRISP keywords/short phrases**, serving as a rapid mental checklist for a non-technical recruiter. Each bullet MUST outline KEY POINTS a candidate should cover and include an indicative score contribution (e.g., 'approx. 2-3 points'). **For some general technical or behavioral questions, especially those testing a core JD requirement like OOP, they must reflect CORE CONCEPTS broken down into their most FUNDAMENTAL, EASILY DIGESTIBLE PARTS (e.g., if OOP is asked, guide the recruiter to check for: '1. Abstraction mentioned. 2. Encapsulation explained. 3. Inheritance example. 4. Polymorphism concept.', etc.).** The goal is a solid, **simple** baseline for evaluation. Answers must be basic, clear, and easy for a non-technical recruiter to evaluate.
    *   **Crucially, all model answers must include guidance on evaluating real-life examples and information shared by the candidate that may not be on their resume, using a note like: 'Note: If the candidate provides relevant real-life examples or discusses experiences/skills not detailed on their resume/profile but clearly relevant to the role, this can indicate greater depth, initiative, or broader experience. The interviewer should assess the relevance and substance of such unstated information against the job requirements.'**
    *   **For the "Tell me about yourself" question, this is an exception to the 'brief keywords' rule.** The model answer MUST be a more descriptive guide FOR THE INTERVIEWER. It should be framed to help a non-technical recruiter assess relevance and completeness against the candidate's documented profile. For example, it should outline the logical structure of a strong intro by pulling specific details from the candidate's profile (e.g., '* Candidate should state their name and university...*', '* Should highlight projects like \\'Project X\\'...*', '* Mention achievements like...*'). Each point should still have an indicative score contribution.
3.  Reflect changes to competency importance, question category, difficulty, or estimated times.
4.  If rubric criteria were changed, reflect these. Ensure criteria are high-quality, actionable, measurable, and usable by non-technical recruiters, referencing JD/profile and considering emergent information. Ensure weights sum to 1.0.
5.  **Question Sequencing**: Aim to preserve or guide towards a logical question sequence.
6.  Ensure all output fields are present for all competencies and questions.

Return the fully customized and refined interview kit in the specified JSON format. The goal is a polished, consistent, and high-quality interview kit that intelligently incorporates the recruiter's edits and adheres to all formatting and contextual requirements, making it **highly usable for non-technical recruiters** and adaptable to information shared during the interview.
`,
});
